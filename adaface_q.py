...
import torch.quantization  # Для квантования модели
...
def load_pretrained_model(architecture='ir_50'):
    ...
    checkpoint = torch.load(adaface_models[architecture], map_location='cpu')  # Загружаем на CPU
...
def quantize_model(model):
    """
    Квантует модель с использованием Dynamic Quantization.
    """
    # Квантуем модель (веса и активации)
...
def main():
    ...
    device = torch.device("cpu")  # Используем CPU для квантованной модели
    ...
    # Квантуем модель
    print("Квантуем модель...")
    quantized_model = quantize_model(model)
    quantized_model = quantized_model.to(device)  # Перемещаем модель на CPU
    ...
    with torch.no_grad():
        for batch_idx, (images, labels) in enumerate(tqdm(dataloader, desc="Извлечение эмбеддингов adaface")):
            if images is None:
                continue
            images = images.to(device)  # Перемещаем данные на CPU
            # Получаем эмбеддинги; модель возвращает кортеж (embedding, norm)
            embedding, _ = quantized_model(images)
